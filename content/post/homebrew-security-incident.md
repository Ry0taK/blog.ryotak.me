...
つまり、`++ "?b/(.*)`にマッチする行であれば、ファイル内容の変更**ではなく**変更中のファイルパス情報として認識されてしまう状態だった。  
...
PyPIやnpm registry等に対する脆弱性診断も行いたいとは思っているのですが、残念ながら脆弱性の診断行為を許可する旨が記載されていないため、現在の所行うことができません。  
