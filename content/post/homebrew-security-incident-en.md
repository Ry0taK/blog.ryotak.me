...
To select the target, I looked at the policy page of the vulnerability disclosure program. And I noticed that `Homebrew/homebrew-*` is in scope.  
...
[^2]: Also, Homebrew had [an incident related to the leakage of GitHub API Token](https://brew.sh/2018/08/05/security-incident-disclosure/) in 2018, so it seems that the awareness of the members was high enough.
...
3. If step 2 didn't match, check if it matches one of the following regular expressions and if it matches, replaces the file path of the source/destination according to the contents.
...
These processes seem to be okay at first glance, but it was possible to change the source/destination file path multiple times in step 3.  
...
[^6]: It is intentional that the second line is not a string literal, because of another bug in `git_diff`, it doesn't ignore double quotes when closing a string literal.
...
[^7]: At this point, I thought that each cask file will be executed only if it was specified in `brew install`.
Given the situation in which the maintainer was in contact, I thought that if it got reverted immediately, no user would execute the modified code.
...
If this vulnerability was abused by a malicious actor, it could be used to compromise the machines that run `brew` before it gets reverted. So I strongly feel that a security audit against a centralized ecosystem is required.  
